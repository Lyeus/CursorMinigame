// Let's go variables
let autoClickerCheck = null;
let autoClicker = null;
let chillClicker = null;
let frenzyClicker = null;
let clicked = false;
let frenzied = false;
let cursorMinigameState = false;
let cursorMinigameCheck = null;
let onlyHumanState = false;
let onlyHumanSpeed = 65;
let clickerLevelBonus = 1;
let filthyCheaterState = false;
...
new Game.Achievement(
 'AUTOCLICKER : READY',
 'Have your automated clicker <b>started.</b><q>Thank you for downloading this addon ! I hope you will enjoy it.</q>',
 [16, 5],
);
Game.last.pool = 'shadow';
new Game.Achievement(
 'Why would I get my cursor ditry ?',
 'Have your automated clicker collect a <b>golden cookie</b>.<q>And then men taught the machine greed.</q>',
 [12, 0],
);
new Game.Achievement(
 "That's the stuff",
 'Have your automated clicker click more than <b>21</b> times per second. <q>Your automated cursor just beated that kid on Youtube that claimed the world record with 21 damn clicks per second ! Damn !</q>',
 [0, 22],
);
Game.last.pool = 'shadow';
new Game.Achievement(
 'Technological singularity',
 "Have your automated clicker beat human's official world record. <b>Twice.</b> <q><b>All will serve roko's basilisk !</b> It's 16 by the way, you ignorant.</q>",
 [18, 5],
);
Game.last.pool = 'shadow';
...
function cursorMinigame() {
 ...
 if (clickerLevelBonus < 1) {
  clickerLevelBonus = 1;
 }
 chillClicker = Math.floor(
  10000 -
   9875 *
    ((Math.floor(clickerLevelBonus) * (Math.floor(Game.Objects.Cursor.amount) - 1)) /
     (Math.floor(clickerLevelBonus) * (Math.floor(Game.Objects.Cursor.amount) - 1) + 39)),
 );
 ...
 if (onlyHumanState && frenzyClicker <= onlyHumanSpeed) {
  frenzyClicker = onlyHumanSpeed;
 }

 // Because he's chilling
 if (!frenzied) {
  autoClicker = chillClicker;
 }
 // Because he's mad
 else {
  autoClicker = frenzyClicker;
 }
...
function filthyCheater(tickSpeed, noImNot) {
 ...
 if (typeof noImNot === undefined) {
  noImNot = false;
 }
 if (typeof tickSpeed === undefined || tickSpeed < 1) {
  tickSpeed = 25;
 }
 ...
 if (filthyCheaterState) {
  ...
  if (noImNot) {
   ...
   console.log('First step is admitting it.');
  }
 } else {
...
function autoClick() {
 if (cursorMinigameState && (!Game.Has('Shimmering veil') || Game.Has('Shimmering veil [on]'))) {
  ...
  Game.shimmers.forEach(function (shimmer) {
   if (
    (shimmer.type == 'golden' || shimmer.type == 'reindeer') &&
    ((clickerLevelBonus >= 2 && !clicked) || filthyCheaterState)
   ) {
    shimmer.pop();
    // Cheaters get the omnipotent click but no achievement. Go cheat your own achievements.
    if (!filthyCheaterState) {
     clicked = true;
     Game.Win('Why would I get my cursor ditry ?');
    }
   }
  });
  ...
  if (!filthyCheaterState) {
   ...
   if (
    ...
    if (!frenzied) {
     ...
     cursorMinigame();
     if (frenzyClicker <= 47) {
      Game.Win("That's the stuff");
     }
     if (frenzyClicker <= 31) {
      Game.Win('Technological singularity');
     }
    }
   }
